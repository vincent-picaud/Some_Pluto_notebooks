...
!!! exemple ""
    **Introduction à la Différentiation Automatique**
    ...
    Exemple d'application: Levenberg-Marquardt (moindres carrés non-linéaires)
...
Cela couvre les problèmes de moindres carrés usuels, où le résidu $r:\mathbb{R}^n\to\mathbb{R}^n$, est de la forme : 
```math
r^i(\theta) = y^i-m(x^i;\theta)
```

L'idée pour résoudre ces problèmes est d'introduire un modèle du second ordre et de remarquer que, souvent (mais pas toujours), une bonne approximation de la matrice Hessienne peut être obtenue à partir de la matrice Jacobienne.
"""

# ╔═╡ 3675b8bb-68ea-4204-9b02-26cbe9148059
md"""
# Calcul différentiel : formule de Taylor

Soit une application $F$ suffisament régulière, on a : 
```math
F(\theta+\delta\theta) = F(\theta) + dF_{|\theta}\cdot \delta\theta + \frac{1}{2}d^2F_{|\theta}\cdot (\delta\theta, \delta\theta) + o(\|\delta\theta\|^2)
```

Il faut marquer une pause pour préciser ce que représente $d^2F$. C'est la différentielle d'ordre deux. Par exemple, si $F:\mathbb{R}^n\to \mathbb{R}$, comme $dF: \mathbb{R}^n\to \mathcal{L}(\mathbb{R}^n,\mathbb{R})$ alors logiquement:
```math
d^2F: \mathbb{R}^n\to \mathcal{L}(\mathbb{R}^n,\mathcal{L}(\mathbb{R}^n,\mathbb{R}))
```
Mais $\mathcal{L}(\mathbb{R}^n,\mathcal{L}(\mathbb{R}^n,\mathbb{R})) \simeq \mathcal{L}(\mathbb{R}^n,\mathbb{R}^n;\mathbb{R})$ espace des formes bilinéaires. 
En effet il suffit de remarquer le jeux d'écriture
```math
(L\cdot x)\cdot y = \tilde{L}\cdot (x,y)
```
qui met en correspondance $L$ avec la forme bilinéaire $\tilde{L}$.

En conclusion, $d^2F$ est identifiée a une forme bilinéaire de $\mathbb{R}^n\times\mathbb{R}^n$ sur $\mathbb{R}$.
"""

# ╔═╡ 723fe95d-5e7c-4126-989d-cda094948bd5
md"""
# Application au problème initial

On cherche une approximation d'ordre deux
```math
F(\theta+\delta\theta) \approx F(\theta) + dF_{|\theta}\cdot \delta\theta + \frac{1}{2}d^2F_{|\theta}\cdot (\delta\theta, \delta\theta)
```

Il faut donc calculer $dF$ et $d^2F$.

## Calcul de $dF$
$dF(\theta\mapsto \frac{1}{2}\|r(\theta)\|^2)$ se calcule par la règle de la chaine : 
```math
dF_{|\theta}\cdot\delta \theta = \langle r(\theta), dr_{|\theta}\cdot \delta \theta\rangle
```
## Calcul de $d^2F$

Rien de mystérieux, on utilise $d^2F=d(dF)$, il vient :
```math
\forall\delta \theta,\ d(\theta\mapsto dF_{|\theta}\cdot\delta\theta)\cdot\delta\lambda = d(\theta\mapsto \langle r(\theta), dr_{|\theta}\cdot \delta \theta\rangle)\cdot\delta\lambda
```
La dérivation du produit scalaire, qui est une forme bilinéaire, est obtenue par la formule de Leibniz (dérivée d'un produit) :
```math
d^2F_{|\theta}\cdot (\delta\theta,\delta\lambda) = 
\langle dr_{|\theta}\cdot \delta\lambda,  dr_{|\theta}\cdot \delta\theta \rangle + \langle r(\theta), d^2r_{|\theta}\cdot (\delta\theta,\delta\lambda)\rangle
```

## Bilan

En notant $J$ la Jacobienne de $r$ en $\theta$, on vient de montrer que :
...
```
et utiliser la méthode de Levenberg-Marquardt classique.
"""
...
!!! note "CAVEAT"
	...
	&\approx F(\theta) + \langle J^t r(\theta), \delta\theta \rangle + \frac{1}{2} \langle J^t J \delta\theta, \delta\theta \rangle
    \end{align}
	```
    C'est plus rapide, (on ignore $d^2F$), mais l'on passe complètement à coté du fait que l'approximation n'est bonne que si le résidu $r$ est faible.
"""
...
begin 
	...
	Y_noisy = Y_true + 0.1*randn(length(X)) # noisy version

	residue(Y,X,θ) = map(((x,y),)->y-peak(x,θ),zip(X,Y))
	# note: le role de ((x,y),) est de destructurer le zip(X,T)
...
begin
	r(θ) = map(((x,y),)->y-peak(x,θ),zip(X,Y_noisy))
	J(θ) = ForwardDiff.jacobian(r,θ)
end
